...
var tf_track = fb.TitleFormat("$if2(%tracknumber%,-)|||%title%|||%track artist%|||[%play_count%]|||[%length%]|||%rating%|||$meta(RATING)");
var tf_rating = fb.TitleFormat("%rating%");
var tf_meta_rating = fb.TitleFormat("$meta(RATING)");
...
var properties = {
    ...
    tag_rating: window.GetProperty("pl: " + __("Use tag rating"), false),
...
// check prop
if (properties.tag_rating || !utils.CheckComponent("foo_playcount", true)) {
    Rating.tag = true;
} else {
    Rating.tag = false;
}
...
function set_rating(to, metadb, tag) {
    var not_set = (!to || to == 0);
    if (tag) {
        metadb.UpdateFileInfoSimple("RATING", not_set ? undefined : to);
    } else {
        fb.RunContextCommandWithMetadb(__("Rating/") + (not_set ? __("<not set>") : to), metadb);
    }
}
...
function get_images() {
    ...
    //images.bg = gdi.Image(fb.ProfilePath + "Skins\\Mnlt2\\bg.jpg");
...
function get_button_images() {
    ...
    var font = gdi.Font("FontAwesome", zoom(16, g_dpi));
    ...
    var font2 = gdi.Font("FontAwesome", z14);
    var w2 = 20;
    // plm_window
    imgarr = [];
    for (s = 0; s < 3; s++) {
...
Playlist = function(mode) {
    ...
    this.padding_h = window.GetProperty("pl: " + __("Padding left/right"), 20);
                    ...
                    case 0:
                        ...
                        */
                        ...
                        if (is_playing) {
                            ...
                            gr.GdiDrawText(fb.IsPaused ? "\uF003" : "\uF002", gdi.Font("mnlt2", z12), txt_color, tn_x, l1y, tn_w, l1h, DT_RC);
                        ...
                        if (properties.show_rating) {
                            ...
                            for (var r = 0; r < 5; r++) {
                                ...
                                var rating_ = Rating.tag ? tags[6] : tags[5];
                                var font = (r < rating_ ? g_fonts.rating1 : g_fonts.rating2);
                                gr.GdiDrawText(r < rating_ ? "\u2605" : "\u2219", font, txt_color, star_x, l2y, star_w, l2h, DT_CC);
                            }
                        }
                        ...
                        switch (true) {
                            ...
                            case (row_type == 0):
                                ...
                                if (properties.show_rating && x > Rating.x && x < Rating.x+Rating.w) {
                                    ...
                                    to = Math.ceil((x - Rating.x) / star_w);
                                    from = Rating.tag ? tf_meta_rating.EvalWithMetadb(metadb) : tf_rating.EvalWithMetadb(metadb);
                                    set_rating(from == to ? 0 : to, metadb, Rating.tag);
                                    break;
                                }
...
function on_size() {
    ...
    var bx = z20, by = (properties.toolbar_height - z25) / 2;
    for (var i = 0; i < 3; i++) {
        bt[i].set_xy(bx, by);
        bx += (bt[i].w + z4);
    }
...
function on_paint(gr) {
    ...
    txt_x = pl.list_x+pl.list_w - txt_w;
