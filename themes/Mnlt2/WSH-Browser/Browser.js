...
// @import "%fb2k_profile_path%Skins\Mnlt2\WSH-Browser\js\JScommon.js"
// @import "%fb2k_profile_path%Skins\Mnlt2\WSH-Browser\js\JSinputbox.js"
// ==/PREPROCESSOR==

// Mod by: Elia
// Update: 2016-01-20
//
// I've modificated this script for my foobar2000 configure `Mnlt2(改)' so it
// would not performed the same as the orginal one.
...
if (lang == "cn") {
    lang_pack = {
		...
		"Warning: this script requires any of the components below:\n": "警告: 此脚本需要以下任何一个组件:\n",
		// Context Menu
		"Refresh": "刷新",
		"Clear cache": "清除缓存",
		"Library": "媒体库",
		"Playlist": "播放列表",
		"Source": "源",
		"Cursor follows Focus": "光标跟随焦点项",
		"Album": "专辑",
		"Genre": "流派",
		"Artist": "艺术家",
		"Columns": "列",
		"Column": "列表",
		"Album Art (bottom labels)": "专辑图片 (底部标签)",
		"Album Art (right labels)": "专辑图片 (右边标签)",
		"Album Art Grid": "专辑图片网格",
		"Header Bar": "顶栏",
		"Aggregate Item": "所有项",
		"Enable Disk Image Cache": "打开图片缓存功能",
		"Display": "显示",
		"Content": "内容",
		"Library selection": "媒体库选中",
		"Library selection (playing)": "媒体库选中(正在播放)",
		"Filter": "过滤器",
		"album": "专辑",
		"Artist": "艺术家",
		"genre": "流派",
...
if (!("FormatDuration" in utils)) {
	fb.trace(__("Warning: this script requires any of the components below:\n") + 
			"==> JScript Panel v1.0.0+" +
			"==> WSH Panel Mod Plus v1.5.7+");
}


images = {
    path: fb.ProfilePath+"\\Skins\\Mnlt2\\WSH-Browser\\images\\",
    glass_reflect: null,
    loading_angle: 0,
    loading_draw: null,
    noart: null,
...
ppt = {
	...
	cachePath: fb.ProfilePath + "Skins\\Mnlt2\\cache\\imgcache\\",
	...
	wallpaperpath: window.GetProperty("_PROPERTY: Default Wallpaper Path", ""),
    ...
    textLineHeight: 0,
    rawBitmap: false,
	selection_alpha: window.GetProperty("_DISPLAY: Selection alpha", 85),
};
...
oFilterBox = function() {
	...
	this.on_init = function() {
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", __("Filter"), g_color_normal_txt, 0, 0, g_color_selected_bg, g_sendResponse, "brw");
...
oScrollbar = function(themed) {
    ...
    this.setCursorButton = function() {
        ...
        if(this.themed) {
            ...
            gb.FillSolidRect(1, 0, this.cursorw-2, this.cursorh, blendColors(this.color_txt, this.color_bg, 0.0) & 0x33ffffff);
        ...
        if(this.themed) {
            ...
            gb.FillSolidRect(1, 0, this.cursorw-2, this.cursorh, blendColors(this.color_txt, this.color_bg, 0.0) & 0x55ffffff);
            //gb.DrawRect(1, 0, this.cursorw-2 - 1, this.cursorh - 1, 1.0, this.color_txt & 0x44ffffff);
        };
        ...
        if(this.themed) {
            ...
            gb.FillSolidRect(1, 0, this.cursorw-2, this.cursorh, blendColors(this.color_txt, this.color_bg, 0.0) & 0x99ffffff);
            //gb.DrawRect(1, 0, this.cursorw-2 - 1, this.cursorh - 1, 1.0, this.color_txt & 0x44ffffff);
        };
...
oBrowser = function(name) {
    ...
    this.sendItemToPlaylist = function(index) {
        ...
        var pidx_playing = -1;
        for(var i = 0; i < total; i++) {
            if(!pfound && plman.GetPlaylistName(i) == __("Library selection")) {
                pidx = i;
                pfound = true;
            };
            //if(!pfound_playing && plman.GetPlaylistName(i) == __("Library selection (playing)")) {
            if(!pfound_playing && plman.GetPlaylistName(i) == __("Library selection")) {
                pidx_playing = i;
                pfound_playing = true;
            };
        ...
        if(utils.IsKeyPressed(VK_CONTROL)) {
            ...
            if(pfound) {
                ...
                plman.CreatePlaylist(pidx, __("Library selection"));
                var from = 0;
            };
            ...
            if(fb.IsPlaying) {
                if(plman.PlayingPlaylist == pidx) { // playing playlist is "Library selection"
                    //plman.RenamePlaylist(pidx, __("Library selection (playing)"));
                    plman.RenamePlaylist(pidx, __("Library selection"));
                    if(pfound_playing) {
                        plman.RenamePlaylist(pidx_playing, __("Library selection"));
                        ...
                        plman.CreatePlaylist(pidx_playing, __("Library selection"));
                    };
    ...
    this.draw = function(gr) {
        ...
        if(repaint_main || !repaintforced){
                ...
                if(ay >= (0 - this.rowHeight) && ay < this.y + this.h) { // if stamp visible, we have to draw it
                    ...
                    if(this.stampDrawMode) {
                        ...
                        if(i == this.selectedIndex) {
                            ...
                            selbg_color = setAlpha(g_color_highlight, ppt.selection_alpha);
                        ...
                        if(i == this.selectedIndex) {
                            txt_color1 = (ppt.enableCustomColors ? g_color_selected_txt : g_color_normal_bg);
                            txt_color2 = blendColors(txt_color1, g_color_selected_bg, 0.25);
                            selbg_color = g_color_highlight & 0xb0ffffff;
                        }; else {
                            ...
                            if(!ppt.showAllItem || (ppt.showAllItem && i > 0) || (ppt.panelMode != 3)) { 
                                if(g_rightClickedIndex > -1) {
                                    if(g_rightClickedIndex == i) {
                                        if(this.stampDrawMode) {
                                            gr.DrawRect(ax+1, ay+1, aw-2, ah-2, 2.0, g_color_highlight & 0xd0ffffff);
                                        }; else {
                                            gr.DrawRect(ax + Math.round((aw - frame_w) / 2) + 1, coverTop + coverWidth - frame_h + 1, frame_w - 3, frame_h - 3, 3.0, g_color_highlight & 0xddffffff);
                                        };
                                    };
                                }; else {
                                ...
                                if(ppt.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
                                    ...
                                    try{
                                        if(ppt.tagMode == 1) {
                                            ...
                                            if(this.groups[i].tracktype != 3) gr.gdiDrawText(arr[0], gdi.Font(g_fname, g_fsize - 1, 0), txt_color2, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth + ppt.botTextRowHeight), coverWidth, ppt.botTextRowHeight + ppt.extra_font_size, DT_LEFT | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
                                        }; else {
                                ...
                                if(ppt.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
                                    ...
                                    try{
                                        if(ppt.tagMode == 1) {
                                            ...
                                            if(this.groups[i].tracktype != 3) gr.gdiDrawText(arr[0], gdi.Font(g_fname, g_fsize - 1, 0), txt_color2, ax+10, (coverTop + 5 + coverWidth + ppt.botTextRowHeight) - ppt.botGridHeight, aw-20, ppt.botTextRowHeight + ppt.extra_font_size, DT_LEFT | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
                                        }; else {
                            ...
                            if(ppt.panelMode == 2) {
                                ...
                                if(this.groups[i].cover_img) {
                                    ...
                                    if(ppt.showAllItem && i == 0 && total > 1) {
                                        ...
                                        gr.DrawRect(ax+this.coverMarginLeft+deltaX, coverTop + deltaY, im_w - 1, im_h - 1, 1.0, g_color_highlight & 0x25ffffff);                                   
                                    };
                                }; else {
                                    var im_w = coverWidth;
                                    var im_h = coverWidth;
                                ...
                                switch(ppt.tagMode) {
                                    case 1: // album
                                        ...
                                        try{
                                            ...
                                            if(this.groups[i].tracktype != 3) gr.gdiDrawText(arr[0], gdi.Font(g_fname, g_fsize - 1, 0), txt_color2, ax + coverWidth + this.marginCover*2, ay + ppt.textLineHeight, aw - coverWidth - this.marginCover*3, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
                                        } catch(e) {}
                                        break;
                                    case 2: // artist
                                        try{
            ...
            if(ppt.showHeaderBar) {
                var item_txt = new Array("", __("album"), __("artist"), __("genre"));
                var nb_groups = (ppt.showAllItem && total > 1 ? total - 1 : total);
                var boxText = nb_groups+" "+item_txt[ppt.tagMode]+(nb_groups > 1 ? __("s  ") : "  ");
                ...
                var tw = this.w - tx + (cScrollBar.enabled ? cScrollBar.width : 0) - 20 * g_zoom_percent/100;
                try {
                    gr.gdiDrawText(boxText, gdi.Font(g_fname, g_fsize-0, 1), blendColors(g_color_normal_txt, g_color_normal_bg, 0.4), tx, 0, tw, ppt.headerBarHeight-1, DT_RIGHT | DT_VCENTER | DT_CALCRECT | DT_NOPREFIX | DT_END_ELLIPSIS);
                }; catch(e) {
            ...
            case "dblclk":
                if(this.ishover) {
                    if(brw.activeIndex > -1) {
						...
						*/
						window.NotifyOthers("Switch panel", 0);
                    };
                }; else {
                    if(cScrollBar.enabled && cScrollBar.visible) {
                        this.scrollbar && this.scrollbar.on_mouse(event, x, y);
	...
	*/
	this.settings_context_menu = function() {};
    ...
    this.item_context_menu = function(x, y, albumIndex) {
			...
			_menu.AppendMenuItem(MF_STRING, 20, __("Refresh"));
			if (albumIndex > -1) {
				_menu.AppendMenuItem(MF_STRING, 21, __("Clear cache"));
			}
			_menu.AppendMenuSeparator();
            
            _menu0.AppendMenuItem(MF_STRING, 50, __("Library"));
            _menu0.AppendMenuItem(MF_STRING, 51, __("Playlist"));
            _menu0.CheckMenuRadioItem(50, 51, 50 + ppt.sourceMode);
            _menu0.AppendTo(_menu,MF_STRING, __("Source"));
            _menu.AppendMenuSeparator();
            
            _menu.AppendMenuItem((ppt.sourceMode == 1 ? MF_STRING : MF_GRAYED | MF_DISABLED), 60, __("Cursor follows Focus"));
            _menu.CheckMenuItem(60, ppt.followFocusChange);
            _menu.AppendMenuSeparator();

            _menu1.AppendMenuItem(MF_STRING, 111, __("Album"));
            _menu1.AppendMenuItem(MF_STRING, 112, __("Artist"));
            _menu1.AppendMenuItem(MF_STRING, 113, __("Genre"));
            _menu1.CheckMenuRadioItem(111, 113, 110 + ppt.tagMode);
            _menu1.AppendTo(_menu,MF_STRING, __("Columns"));
 
            _menu2.AppendMenuItem(MF_STRING, 900, __("Column"));
            _menu2.AppendMenuItem(MF_STRING, 901, __("Album Art (bottom labels)"));
            _menu2.AppendMenuItem(MF_STRING, 902, __("Album Art (right labels)"));
            _menu2.AppendMenuItem(MF_STRING, 903, __("Album Art Grid"));
            _menu2.CheckMenuRadioItem(900, 903, 900 + ppt.panelMode);
            _menu2.AppendMenuSeparator();
            _menu2.AppendMenuItem(MF_STRING, 910, __("Header Bar"));
            _menu2.CheckMenuItem(910, ppt.showHeaderBar);
            _menu2.AppendMenuItem(MF_STRING, 911, __("Aggregate Item"));
            _menu2.CheckMenuItem(911, ppt.showAllItem);
            _menu2.AppendMenuSeparator();
            _menu2.AppendMenuItem(MF_STRING, 912, __("Enable Disk Image Cache"));
            _menu2.CheckMenuItem(912, ppt.enableDiskCache);
            _menu2.AppendTo(_menu,MF_STRING, __("Display"));
			...
			*/
			var _menu10 = window.CreatePopupMenu();
			var Context = fb.CreateContextMenuManager();
			...
			if (albumIndex > -1) {
				...
				_menu.AppendMenuSeparator();
				_menu10.AppendTo(_menu, MF_STRING, __("Content"));
				Context.BuildMenu(_menu10, 2000, -1);
			}
            
            idx = _menu.TrackPopupMenu(x,y);
            
			if (idx >= 2000) {
				Context.ExecuteByID(idx - 2000);
			} else {
				switch(true) {
					case (idx == 20):
						on_key_down(VK_F5);
						break;
					case (idx == 21):
						if(fso.FileExists(ppt.cachePath + crc)) {
							try {
								fso.DeleteFile(ppt.cachePath + crc);
							}; catch(e) {
								fb.trace("WSH Panel Error: Image cache ["+crc+"] can't be deleted on disk, file in use, try later or reload panel.");
							};
						};
        ...
        if(ppt.showHeaderBar) {
            ...
            if(cFilterBox.enabled && g_filterbox) {
                if(g_filterbox.inputbox.visible) {
					var ib_y = (ppt.headerBarHeight - g_filterbox.inputbox.h) / 2;
					g_filterbox.inputbox.font = gdi.Font(g_fname, g_fsize+0);
					g_filterbox.inputbox.font_italic = gdi.Font(g_fname, g_fsize+0, 2);
                    g_filterbox.draw(gr, 20, ib_y);
                };
            };
        };
    };
        ...
        switch(ppt.panelMode) {
            ...
            case 2:
                ...
                if(!timers.mouseWheel) {
                    ...
                    if(ppt.panelMode == 0) {
                        if(ppt.default_lineHeightMin < g_fsize * 3) ppt.default_lineHeightMin = g_fsize * 3;
                        if(ppt.default_lineHeightMin > g_fsize * 10) ppt.default_lineHeightMin = g_fsize * 10;
                    }; else {
                        if(ppt.default_lineHeightMin < g_fsize * 5) ppt.default_lineHeightMin = g_fsize * 5;
                        if(ppt.default_lineHeightMin > g_fsize * 20) ppt.default_lineHeightMin = g_fsize * 20;
                    };
        ...
        if(utils.IsKeyPressed(VK_CONTROL)) { // zoom all elements)
            ...
            if(!timers.mouseWheel) {
                if(step > 0) {
                    ...
                    if(ppt.extra_font_size > 10) ppt.extra_font_size = 10;
                }; else {
                    ppt.extra_font_size -= zoomStep;
                    if(ppt.extra_font_size < 0) ppt.extra_font_size = 0;
                };
...
function get_colors() {
	...
	if (g_color_scheme == 0) {
		...
		g_color_normal_txt = COLOR_SCHEME[g_color_scheme - 1].txt_normal;
		g_color_normal_bg = COLOR_SCHEME[g_color_scheme - 1].bg_normal;
		g_color_selected_txt = COLOR_SCHEME[g_color_scheme - 1].txt_normal;
		g_color_selected_bg = COLOR_SCHEME[g_color_scheme - 1].bg_normal;
		g_color_highlight = COLOR_SCHEME[g_color_scheme - 1].highlight;
	}
...
function on_library_changed(orgin) {
	brw.populate(is_first_populate = false);
}
...
function load_image_from_cache(metadb, crc){
    if(fso.FileExists(ppt.cachePath + crc)){ // image in folder cache
        var tdi = gdi.LoadImageAsync(window.ID, ppt.cachePath + crc);
        return tdi;
    }; else {
        return -1;
    };
...
function save_image_to_cache(metadb, albumIndex) {
    ...
    var comm = "wscript //E:jscript \"" + fb.ProfilePath + "Skins\\Mnlt2\\cache\\LoadIMG.js\" \""
...
on_load();
function on_load(){
    if(!fso.FileExists(fb.ProfilePath + "Skins\\Mnlt2\\cache\\LoadIMG.js")){
        var data = "var fso = new ActiveXObject(\"Scripting.FileSystemObject\");\r\n"
         ...
         + "    Img = IP.Apply(Img);\r\n"
         + "    try{\r\n"
         + "        if(fso.FileExists(WScript.arguments(0) + \"\\\\Skins\\\\Mnlt2\\\\cache\\\\imgcache\\\\\" + crc))\r\n"
         + "            fso.DeleteFile(WScript.arguments(0)+ \"\\\\Skins\\\\Mnlt2\\\\cache\\\\imgcache\\\\\" + crc);\r\n"
         + "        Img.SaveFile(WScript.arguments(0) + \"\\\\Skins\\\\Mnlt2\\\\cache\\\\imgcache\\\\\" + crc);\r\n"
         + "    }catch(err){\r\n"
         + "		return false;\r\n"
         + "    }\r\n"
         + "	return true;\r\n"
        ...
        if(!fso.FolderExists(fb.ProfilePath + "Skins"))
            fso.CreateFolder(fb.ProfilePath + "Skins");
        if(!fso.FolderExists(fb.ProfilePath + "Skins\\Mnlt2"))
            fso.CreateFolder(fb.ProfilePath + "Skins\\Mnlt2");
        if(!fso.FolderExists(fb.ProfilePath + "Skins\\Mnlt2\\cache"))
            fso.CreateFolder(fb.ProfilePath + "Skins\\Mnlt2\\cache");
        if(!fso.FolderExists(fb.ProfilePath + "Skins\\Mnlt2\\cache\\imgcache"))
            fso.CreateFolder(fb.ProfilePath + "Skins\\Mnlt2\\cache\\imgcache");
        var file = fso.CreateTextFile(fb.ProfilePath + "Skins\\Mnlt2\\cache\\LoadIMG.js", true, 65001);
        file.WriteLine(data);
        file.Close();
    };
};
