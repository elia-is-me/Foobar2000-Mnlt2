...
oInputbox = function (w, h, default_text, empty_text, textcolor, backcolor, bordercolor, backselectioncolor, func, parentObjectName) {
    ...
    this.font_italic = gdi.Font("Segoe UI", 12, 2);
    this.w = w;
    this.h = h;
    ...
    this.draw = function (gr, x, y) {
        ...
        this.y = y;
        if (this.edit) {
            var DT = DT_LEFT | DT_VCENTER  | DT_NOPREFIX | DT_CALCRECT;
        } else {
            var DT = DT_LEFT | DT_VCENTER  | DT_NOPREFIX | DT_CALCRECT | DT_END_ELLIPSIS;
        }
        // draw bg
        ...
        if(this.SelBegin!=this.SelEnd) {
            ...
            if( (this.x + px1 + (px2-px1)) > this.x + this.w) {
                gr.FillSolidRect(this.x+px1, this.y+1, this.w-px1, this.h-3, this.backselectioncolor & 0x50ffffff);
            } else {
                gr.FillSolidRect(this.x+px1, this.y+1, px2-px1, this.h-3, this.backselectioncolor & 0x50ffffff);
            }
        } else {
            this.select = false;
            this.text_selected = "";
